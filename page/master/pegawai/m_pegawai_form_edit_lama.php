...
$db = new db();
$conn = $db ->connect();
$sql_edit_peg = $conn -> query("select * from m_pegawai where peg_id='$peg_id'");
$cek=$sql_edit_peg->num_rows;
if ($cek>0) {
	$r=$sql_edit_peg->fetch_object();
?>
		<legend>Edit pegawai <strong><?php echo $r->peg_nama;?></strong></legend>
		...
		<div class="form-group">
			<label for="pegawai_nip" class="col-sm-3 control-label">NIP</label>
				...
				<span class="input-group-addon"><i class="fa fa-tag fa-fw"></i></span>
					<input type="text" name="pegawai_nip" class="form-control" value="<?php echo $r->peg_id;?>" placeholder="NIP Baru" disabled />
				</div>
				</div>
		</div>
		<div class="form-group">
