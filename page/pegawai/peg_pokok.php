...
<legend><i class="fa fa-users" aria-hidden="true"></i> Daftar pegawai</legend>
<?php
$db = new db();
$conn = $db -> connect();
$sql_peg = $conn -> query("select * from m_pegawai order by peg_nik,peg_unitkode asc");
...
$no=1;
	while ($r=$sql_peg->fetch_object()) {
		...
		<tr>
			...
			<a class="btn btn-warning btn-sm btn-block" href="'.$url.'/'.$page.'/edit/'.$r->peg_id.'"><i class="fa fa-pencil-square" aria-hidden="true"></i> edit</a>
			<a class="btn btn-danger btn-sm btn-block" href="'.$url.'/'.$page.'/delete/'.$r->peg_id.'" data-confirm="Apakah data ('.$r->peg_nik.') '.$r->peg_nama.' ini akan di hapus?"><i class="fa fa-trash-o" aria-hidden="true"></i> hapus</a>
			</td>
		</tr>
		';
		$no++;
